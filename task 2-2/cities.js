function getCitiesValues(string) {
  let citiesValues = string
    .split("\n")
    .filter((a) => !(a.startsWith("#") || a === ""))
    .map(function (value) {
      value = value.split(",").filter((a) => !(a.startsWith("#") || a === ""));
  ...
  return (text) => {
    citiesValues.forEach((value, key) => {
      if (text.includes(key)) {
        text = text.replaceAll(
          key,
          `${key} (${value.rating} место в ТОП-10 самых крупных городов Украины, население ${value.population} человек)`
        );
      }
    });
    return text;
...
let string = `44.38,34.33,Алушта,31440,
49.46,30.17,Біла Церква,200131,
49.54,28.49,Бердичів,87575,
46.49,36.58,Бердянськ,121692,
49.15,28.41,Вінниця,356665,
45.40,34.29,Джанкой,43343,#коммент,
...
myFunc = getCitiesValues(string);
console.log(myFunc("Привіт, Київ! Мое місто - Дніпро. Я люблю Київ та Дніпро"));
